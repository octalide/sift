...
`start` is the short form of a subscribe: the repository it names, else the caller's, whole, or one pull request (a number, `#` optional) or one branch (anything else) when `for` names it, under the configured filter, and from a subagent `until: settled` when `for` is set and it names no other `until`. The slash command takes the same arguments in order: `/sift watch start [repo] [for <pr|branch>]`, as in `/sift watch start octalide/sift`, `/sift watch start octalide/sift for 14` and `/sift watch start for feat/14`. `/sift watch subscribe <repo> [scope]` takes the full scope.

`repo` covers the whole repository, `pr <n>` one pull request across every head it moves to (its item events, the runs on its heads and its verdicts), `branch <name>` the runs of one branch, `run <id>` one run, and `tag <glob>` the runs on tags matching the glob (`*` any run of characters, `?` one). `items` (default on) takes issue and pull request events in scope, `stall` (default on) takes `ci stalled`, and `ci` defaults to `watchCi`. A subscription with `until` is removed on its own: `settled` once its verdict (a completed run, outside a `pr` scope) is delivered, or on a `pr` scope once its pull request is merged or closed first, since a closed pull request's head is never settled, `merged` or `closed` once its pull request is, a time once it passes. `until: settled` takes the event that settles it whatever its `ci` filter would hold back, a green verdict or a successful run included, and is refused beside `ci: none`, which delivers nothing that could settle it. A `pr` subscription joins its pull request as it stands: a verdict already out on the open head when it subscribed, on a poller that was running or on the first poll its subscribe starts, is delivered to it alone on the next poll, labelled `ci settled on pr, before it subscribed`. A `run <id>` subscription is removed once its run's completion is delivered, `until` or not, since a run completes only once. A subscription made from a subagent belongs to it and outlives the agent's turn, since a delivery resumes the agent (below). It goes by its `until`, by `unsubscribe`, or once the engine refuses a SendMessage to its agent, which retires every subscription that agent owns. The subscriptions live in the plugin store under the session, so a plugin reload or `/clear` keeps them. When the session ends they are removed with the rest of its watch state, and a session that did not end cleanly (a crash, a killed process) is removed once it has gone unseen for 7 days: a running session marks itself seen every hour, and each session's start removes the others' that are older.
