...
| `post` | `repo`, `kind`, and the fields the write takes: `number`, `tag`, `title`, `body`, `base`, `head`, `draft`, `verdict`, `method`, `target`, `prerelease` | the url of what was written, or the refusal with the rule quoted (see [Outbound text](hooks.md#outbound-text)) |
