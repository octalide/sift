...
Nothing above the code host layer names a host. Every read and write of a repository goes through the `Forge` interface in `src/forge/forge.ts`: the checkout's repository and default branch, the login, issues and their parents (an issue read by number says whether the number names a pull request), comment threads with each commenter's standing and whether that standing maintains the repository, pull requests with their diffs, commits, closing issues, reviews and checks (each with the ci run it belongs to when the forge keeps its log), runs, the command that reads a failed run's log, tags and compares, file trees at a ref (each file with the forge's id for its content) and file contents, which paths are templates, the writes `post` makes on a named repository (each answering the url of what it made), which shell commands write text through its own cli or api, so the outbound gate can refuse them, and where the text sits in a cli write (`cliText`: the command, its body flags and its file flags), so the gate can judge the shell write of a loop that cannot call `post`. A repository is the forge's own path for it, opaque above this layer, and a forge names its artifacts in its own words (`GitHub issue`, `merge request`) for the judge.
