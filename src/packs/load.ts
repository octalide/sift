...
function validateQuestions(raw: unknown, name: string, where: string): Record<string, PackQuestion> {
  ...
  for (const [id, q] of Object.entries(raw as Record<string, PackQuestion>)) {
    ...
    if (q.violates !== undefined) {
      if (q.type === 'noul') throw new Error(`pack ${name}: violates on ${id} needs a choice or a score`);
      if (q.type === 'choice' && (q.violates === 'listed' ? !q.options : !Array.isArray(q.violates) || !q.violates.every((k) => typeof k === 'string'))) {
        throw new Error(`pack ${name}: choice ${id} violates must be a list of option keys, or listed with options`);
      }
      if (q.type === 'score' && (!Array.isArray(q.violates) || !q.violates.every((k) => Number.isInteger(k) && (k as number) >= 0 && (k as number) < q.criteria.length))) {
        throw new Error(`pack ${name}: score ${id} violates must be a list of level indices into its criteria`);
      }
    }
  }
