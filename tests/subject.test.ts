...
import { expectedSubject, parseSubject, type ParsedKind } from '../src/packs/subject.ts';
import { fakeForge } from './fake-forge.ts';

const forge = fakeForge();
// a built-in pack named for the kind it takes
const own = <K extends ParsedKind>(kind: K) => ({ pack: kind, kind });
const refused = (kind: ParsedKind, ref: string | undefined, repo?: string, pack: string = kind) => {
  try {
    parseSubject({ pack, kind }, ref, forge, repo);
  } catch (error) {
...
describe('grade subject', () => {
  it('accepts N, #N and a url for an issue or pull request', () => {
    expect(parseSubject(own('issue'), '80', forge)).toEqual({ kind: 'issue', number: 80 });
    expect(parseSubject(own('issue'), ' #80 ', forge)).toEqual({ kind: 'issue', number: 80 });
    expect(parseSubject(own('pr'), '#7', forge)).toEqual({ kind: 'pr', number: 7 });
    expect(parseSubject(own('issue'), 'https://fake/o/r/issue/80', forge)).toEqual({ kind: 'issue', number: 80, repo: 'o/r' });
    expect(parseSubject(own('pr'), 'https://fake/other/repo/pr/3', forge, 'other/repo')).toEqual({ kind: 'pr', number: 3, repo: 'other/repo' });
    expect(parseSubject(own('pr'), 'dev..HEAD', forge)).toEqual({ kind: 'pr', range: 'dev..HEAD' });
    expect(parseSubject(own('pr'), 'origin/dev...feat/1', forge)).toEqual({ kind: 'pr', range: 'origin/dev...feat/1' });
    expect(() => parseSubject(own('issue'), 'dev..HEAD', forge)).toThrow('not a number');
  });
  ...
  it('accepts a ref or range for a commit and refuses what cannot be one', () => {
    expect(parseSubject(own('commit'), 'HEAD', forge)).toEqual({ kind: 'commit', ref: 'HEAD' });
    expect(parseSubject(own('commit'), 'main..HEAD', forge)).toEqual({ kind: 'commit', ref: 'main..HEAD' });
    expect(parseSubject(own('commit'), 'abc1234', forge)).toEqual({ kind: 'commit', ref: 'abc1234' });
    expect(parseSubject(own('commit'), 'v1.2.0~3^2', forge)).toEqual({ kind: 'commit', ref: 'v1.2.0~3^2' });
  ...
  it('yields a reference, a commit or text for a mixed subject and refuses only what can be nothing', () => {
    expect(parseSubject(own('mixed'), '42', forge)).toEqual({ kind: 'mixed', subject: { kind: 'issue', number: 42 } });
    expect(parseSubject(own('mixed'), ' #42 ', forge)).toEqual({ kind: 'mixed', subject: { kind: 'issue', number: 42 } });
    expect(parseSubject(own('mixed'), 'https://fake/o/r/issue/80', forge)).toEqual({ kind: 'mixed', subject: { kind: 'issue', number: 80, repo: 'o/r' } });
    expect(parseSubject(own('mixed'), 'https://fake/o/r/pr/3', forge, 'o/r')).toEqual({ kind: 'mixed', subject: { kind: 'pr', number: 3, repo: 'o/r' } });
    expect(parseSubject(own('mixed'), 'abc1234', forge)).toEqual({ kind: 'mixed', subject: { kind: 'commit', ref: 'abc1234' } });
    expect(parseSubject(own('mixed'), 'main..HEAD', forge)).toEqual({ kind: 'mixed', subject: { kind: 'commit', ref: 'main..HEAD' } });
    expect(parseSubject(own('mixed'), 'the watcher misses body edits', forge)).toEqual({ kind: 'mixed', subject: { kind: 'text', text: 'the watcher misses body edits' } });
    expect(parseSubject(own('mixed'), 'NaN', forge)).toEqual({ kind: 'mixed', subject: { kind: 'text', text: 'NaN' } });
  ...
  it('reads an issue or free text for the rules and refuses a pull request or a commit, naming what it takes', () => {
    expect(parseSubject(own('rules'), '42', forge)).toEqual({ kind: 'rules', subject: { kind: 'issue', number: 42 } });
    expect(parseSubject(own('rules'), ' #42 ', forge)).toEqual({ kind: 'rules', subject: { kind: 'issue', number: 42 } });
    expect(parseSubject(own('rules'), 'https://fake/o/r/issue/80', forge)).toEqual({ kind: 'rules', subject: { kind: 'issue', number: 80, repo: 'o/r' } });
    expect(parseSubject(own('rules'), 'the watcher misses body edits', forge)).toEqual({ kind: 'rules', subject: { kind: 'text', text: 'the watcher misses body edits' } });
  ...
  it('names the pack that was asked, not the kind of subject it parses', () => {
    expect(refused('mixed', undefined, undefined, 'locate')).toBe(`locate pack: no subject; expected ${expectedSubject('mixed', forge)}`);
    expect(refused('mixed', 'https://elsewhere.example/o/r/issues/3', undefined, 'locate')).toMatch(/^locate pack: subject is a URL Fake does not serve/);
    expect(refused('issue', 'NaN', undefined, 'plan')).toBe(`plan pack: subject is not a number ("NaN"); expected ${expectedSubject('issue', forge)}`);
    expect(refused('rules', 'abc1234', undefined, 'house-rules')).toMatch(/^house-rules pack: subject is a commit ref/);
  });

  it('accepts a tag or "release" for a release and refuses what cannot be one', () => {
    expect(parseSubject(own('release'), 'release', forge)).toEqual({ kind: 'release' });
    expect(parseSubject(own('release'), 'v1.4.0', forge)).toEqual({ kind: 'release', proposed: 'v1.4.0' });
    expect(parseSubject(own('release'), '2026.9', forge)).toEqual({ kind: 'release', proposed: '2026.9' });
  ...
  it('parses the github url shapes of an issue or pull request', () => {
    ...
    expect(parseSubject(own('issue'), 'https://github.com/octalide/sift/issues/80', gh)).toEqual({ kind: 'issue', number: 80, repo: 'octalide/sift' });
  });
});
