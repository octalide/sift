...
describe('pruning', () => {
  ...
  it('notes a dropped Read range in file lines with the call to read it back', async () => {
    ...
    expect(result.text).toContain('[sift: lines 240-299 (60 lines) omitted as not needed for the current task, re-read /src/a.ts with offset 240 limit 60, or call mcp__sift__prune off to read files whole]');
  ...
  it('notes real line numbers around a 5000-character line in Read output', async () => {
    ...
    // a Read loses lines only at its tail, so the low chunks in front of chunk 6 stay and every kept line keeps its number
    expect(result.text).toBe([...text.split('\n').slice(0, 138), '[sift: lines 238-299 (62 lines) omitted as not needed for the current task, re-read /src/a.ts with offset 238 limit 62, or call mcp__sift__prune off to read files whole]'].join('\n'));
    const lines = result.text.split('\n');
    expect(lines[120]).toBe(long);
    expect(lines[121]).toBe('line 121');
  });
});
