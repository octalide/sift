...
import { applyDecisions, collectCalls, compact, reduction, type Message } from '../src/compact/compact.ts';
...
describe('compaction', () => {
  ...
  it('drops stale calls, truncates unneeded results, and keeps text and handles', async () => {
    ...
    const result = await compact(messages, judgeBy((id) => (id === 'keep_t1' ? 0.1 : id === 'full_t2' ? 0.2 : 0.9)), {
      keepThreshold: 0.5,
      pinRecent: 2,
      truncateHead: 5,
